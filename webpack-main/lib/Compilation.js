...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
   ...
   */
  //  2.主要是通过 _addEntryItem添加入口,因为编译时从入口开始
  ...
  _addEntryItem (context, entry, target, options, callback) {
    ...
    //  4.调用 addModuleTree 将当前的模块加入到 module tree(模块树)
    this.addModuleTree(
      {
        context,
        dependency: entry,
  ...
  _computeAffectedModules (modules) {
    ...
    this.logger.log(
      `${Math.round(
        ...
        this.modules.size
      )}% (${affectedModules.size} affected + ${infectedModules.size
      } infected of ${this.modules.size
    ...
    if (this.profile) {
      ...
      const logNormalSummary = (category, getDuration, getParallelism) => {
        ...
        for (const [module, profile] of modulesWithProfiles) {
          ...
          logByValue(
            t,
            ` | ${Math.round(t)} ms${p >= 1.1 ? ` (parallelism ${Math.round(p * 10) / 10})` : ""
      ...
      const logByLoadersSummary = (category, getDuration, getParallelism) => {
        ...
        for (const [key, modules] of map) {
          ...
          logByValue(
            t,
            ` | ${Math.round(innerSum)} ms ${category} > ${loaders
              ? `${modules.length
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
  ...
  createModuleHashes () {
    ...
    this.logger.log(
      `${statModulesHashed} modules hashed, ${statModulesFromCache} from cache (${Math.round(
