...
class Compiler {
  ...
  run (callback) {
    ...
    //  定义了onCompiled函数,主要传给this.compile作为执行的回调函数
    ...
    const run = () => {
      //  beforeRun阶段注册的 plugin 会在此处执行
      this.hooks.beforeRun.callAsync(this, err => {
        if (err) return finalCallback(err);
        //  run阶段注册的 plugin 会在此处执行
...
${other}`);
  ...
  createCompilation (params) {
    ...
    //  通过 new Compilation 创建 compilation
  ...
  newCompilation (params) {
    //  调用 this.createCompilation() 方法,返回compilation实例
  ...
  compile (callback) {
    ...
    this.hooks.beforeCompile.callAsync(params, err => {
      ...
      //  传入params调用newCompilation,实例化compilation对象
      const compilation = this.newCompilation(params);

      const logger = compilation.getLogger("webpack.Compiler");

      logger.time("make hook");
      // 🅾 make 勾子,真正使用compilation 对模块执行编译
      this.hooks.make.callAsync(compilation, err => {
        ...
        this.hooks.finishMake.callAsync(compilation, err => {
          ...
          process.nextTick(() => {
            ...
            //  执行ompilation的finish方法,对module上的错误或警告处理
            compilation.finish(err => {
              logger.timeEnd("finish compilation");
              if (err) return callback(err);

              logger.time("seal compilation");
              //  执行seal方法,对module代码进行封装输出
